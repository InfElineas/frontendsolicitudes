...
const normalizeStatus = (raw) => STATUS_LABELS[String(raw || '').trim().toLowerCase()] || raw;
...
export const deriveHistory = (data) => {
  ...
  if (initial) {
    timeline.push({ at: initial.toISOString(), from: null, to: 'Creado / Solicitado' });
  }
  ...
  if (statusMap && typeof statusMap === 'object') {
    Object.entries(statusMap).forEach(([status, at]) => {
      ...
      if (parsed) timeline.push({ at: parsed.toISOString(), from: null, to: normalizeStatus(status) });
    });
  }
  ...
  knownTimestampKeys.forEach(({ key, label }) => {
    ...
    if (parsed) timeline.push({ at: parsed.toISOString(), from: null, to: label });
  });
  ...
  return ordered.map((entry, idx) => {
    ...
    const durationMs = currentDate && nextDate ? nextDate.getTime() - currentDate.getTime() : null;
    return {
      ...entry,
      durationMs,
