...
jest.mock('./CreateRequestDialog', () => () => <div data-testid="create-request-dialog" />);
...
const getTextCount = (container, text) =>
  Array.from(container.querySelectorAll('*')).filter((el) => el.textContent.trim() === text).length;

const getButtonsByLabel = (container, text) =>
  Array.from(container.querySelectorAll('button')).filter((btn) => btn.textContent.includes(text));
