...
import { FileText, Clock, CheckCircle, TrendingUp, Settings2, UserPlus, Users, BarChart3 } from 'lucide-react';
...
function AnalyticsView({
  ...
  const maxTotal = useMemo(() => Math.max(
    ...filtered.map((row) => (Number(row.assigned) || 0)
  ...
  const technicianOptions = users.length > 0
    ? users
    : (analytics?.productivity_by_tech || []);
            ...
            <SelectContent>
              ...
              {technicianOptions.map((tech) => (
                <SelectItem
                  ...
                  value={String(tech.id || tech.user_id || tech.username || tech.name)}
                >
      ...
      {analytics && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <SummaryCard icon={<FileText className="h-4 w-4 text-muted-foreground" />} label="Total de solicitudes" value={global.assigned} sub="Asignadas en el periodo" />
            <SummaryCard icon={<CheckCircle className="h-4 w-4 text-green-600" />} label="Finalizadas" value={global.finished} sub="Cerradas" valueClass="text-green-600" />
            <SummaryCard icon={<Clock className="h-4 w-4 text-blue-600" />} label="En proceso" value={global.inProgress} valueClass="text-blue-600" />
            <SummaryCard icon={<TrendingUp className="h-4 w-4 text-muted-foreground" />} label="En revisión" value={global.inReview} />
            <SummaryCard icon={<Settings2 className="h-4 w-4 text-orange-600" />} label="Pendientes" value={global.pending} valueClass="text-orange-600" />
            <SummaryCard icon={<UserPlus className="h-4 w-4 text-muted-foreground" />} label="Promedio por técnico" value={global.averagePerTech.toFixed(1)} sub="Asignadas" />
            <SummaryCard icon={<Users className="h-4 w-4 text-muted-foreground" />} label="Técnicos activos" value={global.technicianCount} sub="Con datos en el periodo" />
            <SummaryCard icon={<BarChart3 className="h-4 w-4 text-muted-foreground" />} label="Finalizadas por técnico" value={global.averageFinishedPerTech.toFixed(1)} sub="Promedio" />
          </div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Distribución de estados para cada técnico según filtros y periodo seleccionado.
              </CardDescription>
            </CardHeader>
            <CardContent>
              {(!noData) ? (
                    ...
                    <tbody>
                      {filtered.map((row) => (
                        <tr key={row.user_id}>
                          ...
                          <td className="py-2 pr-2 text-right">{row.inProgress ?? 0}</td>
                          <td className="py-2 pr-2 text-right">{row.inReview ?? 0}</td>
                          <td className="py-2 pr-2 text-right">{row.finished ?? 0}</td>
                          <td className="py-2 pr-2 text-right">{row.pending ?? 0}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Comparativa de finalización por persona</CardDescription>
              </CardHeader>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Asignadas, en proceso, revisión y pendientes por técnico</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                {(!noData) ? (
                  filtered.map((row) => (
...
const BarItem = ({ label, value = 0, max = 0, colorClass }) => {
  ...
  const percentage = Math.min(100, Math.max((value / clampedMax) * 100, value > 0 ? 6 : 0));
  return (
...
const StackedBar = ({ row, max = 0 }) => {
  ...
  const total = segments.reduce((acc, seg) => acc + (Number(row[seg.key]) || 0), 0);
